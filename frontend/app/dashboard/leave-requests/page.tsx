...
export default function LeaveRequestsPage() {
  ...
  const handleApprove = async (id: string) => {
    try {
      await feathersClient.service('leave-requests').patch(id, { status: 'approved' });
      fetchLeaveRequests();
    } catch (error) {
  ...
  const calendarEvents = filteredRequests.flatMap(request => {
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      events.push({
        ...
        date: new Date(d),
        title: `${request.worker?.firstName} ${request.worker?.lastName} - ${request.leaveType?.name || 'Leave'}`,
                ...
                {filteredRequests.map((request) => (
                  <tr key={request._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">{request.worker?.employeeId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-900">{request.leaveType?.name || 'N/A'}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
